...
describe("RubricEvaluationRunner", () => {
  it("Claude·Codex evaluator는 CLI가 없는 maxTurns를 비우고 Grok만 턴을 제한한다", () => {
    const claude = evaluatorConfig("Claude judge", "claude").config;
    const codex = evaluatorConfig("Codex judge", "codex").config;
    const grok: ExperimentEvaluatorConfig = {
      label: "Grok judge", provider: "grok", accountId: null, model: "grok-judge", reasoningEffort: null, family: "grok",
    };

    expect(buildEvaluatorVariantConfig(claude, 2).runtime.maxTurns).toBeNull();
    expect(buildEvaluatorVariantConfig(codex, 1).runtime.maxTurns).toBeNull();
    expect(buildEvaluatorVariantConfig(grok, 1).runtime.maxTurns).toBe(3);
  });
...
describe("블라인드 피험 패킷", () => {
  ...
  it("Grok·xAI 지문도 마스킹한다", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "wam-blind-packet-grok-"));
    roots.push(root);
    fs.writeFileSync(path.join(root, "result.txt"), "before\n");
    const { execFileSync } = await import("node:child_process");
    execFileSync("git", ["init", "--quiet"], { cwd: root });
    execFileSync("git", ["add", "result.txt"], { cwd: root });
    execFileSync("git", ["-c", "user.name=test", "-c", "user.email=test@example.com", "commit", "-m", "base", "--quiet"], { cwd: root });
    fs.writeFileSync(path.join(root, "result.txt"), "Generated by Grok 4.6\n");
    fs.writeFileSync(path.join(root, "note.txt"), "xAI Grok wrote this\n");

    const packet = await buildBlindSubjectPacket({
      blindLabel: "후보-B", taskCommand: "구현", finalAnswer: "Generated by Grok 4.6", workingDirectory: root,
    });

    expect(JSON.stringify(packet)).not.toMatch(/Grok|xAI/i);
    expect(packet.leakageRedactions).toBeGreaterThanOrEqual(3);
  });
});
