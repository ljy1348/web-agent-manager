...
description: 현재 작업을 web-agent-manager의 다른 Claude·Codex·Grok 자식 채팅에 맡기고 완료 결과를 회수한다. 명시적 위임 요청뿐 아니라 다른 공급자의 독립 구현·검토가 정확도를 실질적으로 높이는 복잡한 작업에도 사용한다.
---
...
2. 대상 채팅 번호가 있으면 `targetChatId`를 그대로 사용한다. 번호가 없으면 프로젝트와 다른 `provider`(`codex`·`claude`·`grok`)를 명시하고 `createNew: true`로 독립 자식 채팅을 만든다.
...
```bash
WEB_AGENT_MANAGER_ROOT="$(dirname "$(dirname "$(realpath .agents/skills/web-agent-manager-delegate 2>/dev/null || realpath .claude/skills/web-agent-manager-delegate 2>/dev/null || realpath .grok/skills/web-agent-manager-delegate)")")"
