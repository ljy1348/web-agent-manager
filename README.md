...
- 데스크톱과 모바일에서 같은 네이비·바이올렛 디자인 체계, 아이콘 탭 내비게이션, 명확한 활성/포커스 상태와 44px 안팎의 모바일 조작 영역을 사용한다. 자주 쓰는 모바일 하단은 `대시보드`·`채팅`·`파일`·`GitHub`·`더보기` 5개를 고정하고, 지침·예약·실험실·도구·설정은 설명이 붙은 더보기 시트에서 연다. 비채팅 화면에도 현재 프로젝트 선택을 유지하고, 채팅은 목록·대화·터미널·입력 영역을 한 화면 높이 안에서 독립적으로 관리한다
- 대시보드 런타임 카드에서 관리자가 Codex·Claude·Grok CLI를 공식 업데이트 명령으로 갱신할 수 있다. 성공 후 해당 공급자의 실행 중 채팅과 계정별 모델 목록·사용량 조회 PTY를 모두 재시작해 새 바이너를 즉시 쓰며, 진행 중인 응답이 중단될 수 있음을 확인창에 명시한다
- 프로젝트 추가 화면에서 설정된 워크스페이스 경로와 폴더명을 지정해 새 디렉터리와 `main` Git 저장소를 한 번에 만들거나, 기존 폴더 등록·GitHub 저장소 clone을 선택한다. 새 프로젝트와 기존 폴더 모두 선택적으로 GitHub 저장소를 생성해 `origin`을 연결할 수 있고 Private·Public·Internal 공개 범위를 고른다
- `예약` 탭에서 매일 지정 시각·IANA 시간대·입력 문구를 영구 저장한다. 매 실행마다 선택한 Codex·Claude·Grok 공급자의 새 채팅을 만들거나 지정한 기존 채팅에 이어서 입력하며, 일정 수정·일시정지·즉시 실행·삭제와 최근 생성 채팅 이동을 지원한다. 서버가 예약 시각에 꺼져 있었다면 같은 현지 날짜 안에 다시 켜졌을 때 한 번 실행하고, 실행일을 SQLite에서 먼저 선점해 재시작이나 겹친 검사에도 중복 전송하지 않는다. `실행 날짜`를 넣으면 매일 반복 대신 그 날짜·시각에 한 번만 실행하고 자동으로 꺼지며(서버가 그때 꺼져 있었다면 다시 켜진 뒤 한 번), 이미 지난 날짜·시각의 1회 예약은 즉시 실행 사고를 막기 위해 저장·재활성화를 거부한다. 1회 예약을 `지금 실행`하면 그 한 번을 쓴 것으로 보고 끈다
- 채팅 목록 맨 위 `실행 중 · 전체 프로젝트` 패널은 프로젝트와 상관없이 터미널이 살아 있거나 작업 중·리밋 대기인 채팅을 프로젝트별 소제목 아래 한 줄 행(상태 점·제목·제공자, 권한 요청·리밋 대기·오류만 칩)으로 모아 보여주고, 접어도 확인이 필요한 개수는 헤더에 남기며, 누르면 그 프로젝트로 전환해 채팅을 연다. 채팅 상태·기록·승인 실시간 이벤트가 오면 짧게 모아 다시 불러온다
...
- Codex 내부 권한 상승 검토용 JSONL과 실제 사용자 메시지가 없는 Claude 상태 조회·local-command 전용 JSONL 세션은 일반 채팅 목록에 노출하지 않음. 일반 웹 질문은 첫 user 메시지와 전송 대상 claim을 정확히 대조해 동시에 떠 있는 다른 빈 채팅에 세션이 붙지 않으며, 실제 user/assistant 기록이 끝까지 없는 채팅은 터미널 종료 시 목록에서도 자동 제거
- 응답은 마크다운(GFM: 표·코드블록·목록 등)으로 렌더링하고, 코드 블록 오른쪽 위 `복사` 버튼으로 명령을 드래그 없이 복사(모바일은 아이콘만), 도구 실행·diff 상세는 체크박스를 켜야만 노출(기본은 완전히 숨김, 켜면 실제로 펼쳐짐)하고 diff는 GitHub·git처럼 줄 색상으로 표시. 대화 가상 스크롤은 하단을 보고 있을 때 새 답변을 계속 따라가되 wheel·touch·pointer 입력이 시작되면 자동 추적을 즉시 멈추고 과거 위치를 보존하며, 실제 사용자 스크롤에서만 이전 메시지를 추가로 불러옴
...
- Alt+Enter 또는 Ctrl+Enter로 메시지 전송, 전송 즉시 사용자 메시지를 화면에 먼저 표시하고 JSONL 반영이 확인되면 실제 메시지로 교체. tmux의 bracketed paste와 공급자별 붙여넣기 안정화 지연을 사용하며, Codex·Claude·Grok은 유휴·작업 중 모두 붙여넣은 글이 입력창에 보인 뒤 Enter를 보내고, 입력창이 비워짐(작업 중이면 대기열 진입)·UserPromptSubmit 훅·JSONL 기록 중 하나로 제출을 확인한다. 글이 입력창에 계속 남아 있을 때만 Enter를 최대 3번 재시도하고, 6초 안에 끝내 남아 있으면 거짓 `작업중`과 남은 초안을 원복한다(기록이 늦게 써지는 것만으로는 실패로 보지 않음). 새 TUI는 입력 프롬프트가 준비된 뒤에만 첫 메시지를 보내며, 작업 중인 Codex·Claude·Grok에는 후속 질문·명령을 TUI 큐로 보내고 중지 버튼도 함께 제공. 작성 중인 텍스트는 채팅별 브라우저 메모리에 두어 WAM 내부 탭·채팅을 왕복해도 유지하며, 전송·로그아웃 때 지운다. 새로고침이나 브라우저 종료 뒤까지 영구 저장하지는 않는다
...
- Grok(`grok` CLI)은 채팅 코어 범위에서 Codex·Claude와 같은 방식으로 동작한다. 새 채팅·resume, 기록 자동 발견과 대화 동기화, 작업중·완료 표시, 도구 실행 승인, 모델·추론 강도 변경, 권한 모드 표시, 계정 슬롯(`GROK_HOME`)까지 지원한다. Grok은 세션을 파일 하나가 아니라 디렉터리(`~/.grok/sessions/<URL인코딩 cwd>/<세션UUID>/`)로 저장하고 대화 본문은 `chat_history.jsonl`, 턴 종료는 `events.jsonl`의 `turn_ended`에만 남기므로 두 파일을 함께 읽어 완료를 판정한다(도구 호출 직전 설명 문장을 완료로 오인하지 않기 위함). 승인 화면은 선택지 구성이 도구마다 달라 고정 번호를 쓰지 않고 화면에 실제로 뜬 선택지 문구에서 번호를 찾아 누른다. 사용량은 `/usage show` 화면에서 플랜 주간 한도(예: `Weekly limit (SuperGrok)`)의 사용률과 초기화 시각을 읽고, 같은 화면의 세션 토큰·호출·비용 줄을 참고 활동으로 함께 남긴다(`/usage`만 보내면 show·manage 하위 선택에서 멈추고, 이 화면은 입력창을 덮는 모달이라 파싱 뒤 Esc로 닫아야 다음 주기 조회가 입력창에 들어간다). 응답별 말풍선 토큰은 `updates.jsonl`의 `turn_completed.usage`를 그 턴의 마지막 assistant에 붙여 Claude·Codex와 같은 원장 경로로 저장한다. `chat_history.jsonl`에는 usage가 없고, Grok 숫자는 Codex처럼 입력에 캐시 읽기가 포함되며 합계는 다시 더하지 않는다. 자식 채팅 위임도 Codex·Claude와 동일하게 지원한다(위임 대상 공급자는 등록된 어댑터 목록으로 판정하므로 Grok 채팅을 대상으로 지정하거나 새로 만들 수 있다). 전역 스킬·MCP 자동 연동도 지원한다(스킬은 `~/.grok/skills`, MCP는 `grok mcp add --scope user`로 등록하며, Grok CLI에는 `mcp get`이 없어 연동 확인은 `grok mcp list` 출력으로 한다). 에이전트 실험실(Agent Lab)에서도 Grok을 variant·보조 런타임·evaluator로 돌릴 수 있다(`grok -p <프롬프트> --output-format streaming-messages-json`을 쓰며, 이 출력이 Claude `stream-json`과 같은 wire format이라 이벤트 변환기를 공유한다). 다만 Grok headless(`-p`)에는 스킬을 끄거나 고정하는 수단이 없어(플러그인 주입 `--plugin-dir`는 `grok agent` 전용이라 headless가 거부한다) 스킬 격리 비교 자체를 지원하지 않고 현재 설치 스킬셋 그대로 실행한다. 비용 상한 옵션도 없어 예산은 WAM 정책만 강제하며, OS 샌드박스(`--sandbox`)는 Linux에서 bubblewrap이 있을 때만 걸고 없으면 권한 모드로만 돌린다(강제 수단이 무엇이었는지는 실행 스냅샷에 남는다)
...
- 기록이 있는 종료 채팅은 웹에서 "터미널 시작" 버튼으로 다시 시작하거나, 입력 시 파일 내부 ID까지 검증된 공급자 세션으로 자동 resume. 기록 파일이 없는 예약 UUID는 폐기하고 새 세션으로 시작해 Claude의 `No conversation found` 즉시 종료를 막음
...
- Codex TUI 승인 호환 계층, Claude·Codex `PermissionRequest` 훅 기반 웹 승인(Codex는 명령 실행·파일 수정 승인을 훅으로 받아 1회 허용·거부 카드로 보여주고, WAM 서버가 응답하지 못하면 Codex 기본 승인 화면으로 넘어가 기존 화면 감지가 이어받음)과 훅 밖 rate-limit 선택·대형 세션 resume 선택·디렉터리 신뢰 확인(y/n)·경량 모델 전환 화면 감지(요청유형에 맞는 버튼 라벨과 알림 요약 표시), 선택 채팅 안에서 인라인 승인 처리. Claude 최종 응답이 기록되면 유실된 훅 대기를 자동 정리하고 완료 채팅의 잔여 요청은 안전하게 닫을 수 있음. Claude 훅은 tmux가 넘긴 채팅 ID를 함께 보내 같은 프로젝트에 Claude 채팅이 여럿이어도 승인 카드가 정확한 채팅에 붙는다. 웹에서 띄운 Claude는 `UserPromptSubmit`·`Stop`·`StopFailure` HTTP 훅으로 턴 시작·종료를 알려 작업중 표시는 훅에서 바로, 완료는 2초 폴링을 기다리지 않고 즉시 반영된다. 관리자는 `GET /api/admin/hook-stats`로 폴링이 찾은 턴 종료마다 Stop 훅이 제때 왔는지(일치·누락) 볼 수 있다(훅이 실패해도 Claude 작업은 막히지 않고 기존 폴링이 이어서 반영). Claude에서 `/clear` 등으로 새 세션이 시작되면 `SessionStart` 훅으로 같은 웹 채팅을 새 대화에 옮겨 붙인다. 웹에서 띄운 Codex 채팅도 WAM 훅(`SessionStart`·`UserPromptSubmit`·`Stop`)으로 첫 기록을 session ID로 정확히 연결하고 턴 종료를 즉시 반영한다 — Codex는 비관리 훅을 해시 신뢰 없이 실행하지 않아 WAM 채팅 실행에만 `--dangerously-bypass-hook-trust`를 붙이며(그 세션에서는 저장소 `.codex`의 다른 훅도 검토 없이 실행됨), 사용량 조회 PTY에는 붙이지 않는다. Grok은 신뢰 절차 없이 로드되는 전역 훅만 쓸 수 있어, 서버가 시작할 때 `~/.grok/hooks/web-agent-manager.json`을 설치해 턴 시작·종료·중단과 세션 ID를 받는다. 이 파일은 사용자가 직접 띄운 Grok 세션도 읽지만, 채팅 ID 환경변수가 없는 세션에서는 브리지가 아무것도 하지 않는다. 데스크톱 권한 요청 패널은 현재 프로젝트에 대기 요청이 있을 때만 나타나고 평소에는 채팅 본문이 해당 폭을 사용
- 공급자별 전용 상태 PTY, 60초 주기 사용량 및 reset 시각 조회(Claude 상태 PTY는 인증·내장 명령만 남기는 safe mode와 screen-reader 출력으로 실행). 상태 PTY는 3시간마다 새 프로세스로 갈아탄다 — 오래 켜두면 Claude CLI가 `/usage`에 프로세스 시작 시점의 캐시를 간헐적으로 돌려주는 것이 실측됐고(16시간 된 PTY가 최신값과 시작 당시 값을 매분 번갈아 반환), 갈아타는 동안에는 마지막 정상값을 유지해 화면에서 사용량이 사라지지 않는다. Claude `/usage`가 `Per-model breakdown unavailable (rate limited)` 재시도 화면이면 그 숫자는 시작 시점 캐시일 수 있어(실측: 실제 채팅 19%인데 조회 PTY 1%) 성공 스냅샷으로 쓰지 않고 마지막 정상값을 유지한다. 실제 대화가 없어 Claude 5시간 세션 창이 조회에서 사라지거나 0%가 되고, Codex 창이 0%가 되면 새 초기화 창마다 별도 임시 PTY에 유지 턴을 보내 롤링 초기화 시각을 실제 활동에 고정한다. Codex keepalive는 격리 `codex exec --json`으로 보내고, 완결된 답과 `turn.completed`를 확인한 뒤에만 종료한다. 확인 실패마다 요구 응답 길이를 1500에서 최대 10회 만에 10000자까지 늘리며, 10회를 다 쓰면 세션 유지 실패 알림을 보내고, 5시간 창이 이미 1% 이상이면 더 보내지 않고 현재 창을 성공으로 남긴다. 응답 도착만으로 성공 처리하지 않는다. 후속 사용량에서 대상 창 하나라도 `1% 이상`이면 즉시 고정 성공으로 확정하며, 주간 창처럼 짧은 턴이 0%로 반올림되는 경우에는 충분한 간격의 두 조회에서 reset 시각이 멈춘 경우만 보조 성공으로 인정한다. 0%와 reset 이동이 계속되면 성공 전송 시각을 기록하지 않고 10분 뒤 한국어 다섯 문장·300자 이상 문구로 재시도한다. Claude는 기존 `hi` 최소 턴과 즉시 성공 계약을 유지한다. 반복 `/usage`가 쌓인 장수 조회 세션을 모델 턴에 재사용하면 JSONL 실측상 다음 최소 턴의 캐시 생성 입력이 약 2만 토큰씩 증가하므로, 최소 턴은 매번 깨끗한 임시 상태 PTY에서 실행하고 응답 직후 폐기한다. 초기화 절대시각 기반 창 키를 DB에 저장해 같은 창의 반복 조회·서버 재시작 중복은 막되 새 창이면 직전 전송 후 5시간이 지나지 않아도 즉시 보낸다. 이 내부 기록은 일반 채팅 목록에서 숨기고 대시보드 사용량 카드에는 마지막 확인 성공 시각과 감지 사유를 표시한다. Codex 초기화권은 `/usage`의 Full reset 상세와 공식 app-server 응답을 하루 1회 확인해 잔여 개수·가장 이른 기한을 저장하고, 중간 조회가 기한 없는 개수만 반환해도 마지막 정상 기한을 보존. 관리자는 대시보드 확인창을 거쳐 해당 계정 카드의 맨 위 Full reset 초기화권을 공식 app-server 메서드로 사용할 수 있고, 서버가 계정·실제 잔여량·중복 요청·결과를 다시 검증한다. 대시보드에 마지막 파싱 시각·Codex 초기화권·카드별 새로고침 버튼(즉시 일반 사용량 재조회 요청)을 표시하고, CLI가 옛 스냅샷을 돌려줘 같은 창의 사용량이 뒤로 후퇴하면 마지막 정상값을 유지한 채 stale로만 표시하되, 같은 거부가 5회(약 5분) 연속되면 그 값이 실제 최신값이라고 보고 채택해 오래된 값이 무기한 굳지 않게 함. 플랜을 바꾸면 조회 PTY가 시작 시점 값을 계속 돌려줘 새로고침으로는 갱신되지 않으므로, 카드의 `터미널 재시작` 버튼으로 해당 공급자의 조회 전용 PTY만 즉시 교체할 수 있다(실행 중인 채팅은 재시작하지 않음). Codex Pro 플랜 `/status`는 플랜 한도 아래에 값 없는 모델 전용 헤더(예: `GPT-5.3-Codex-Spark limit:`)와 그 모델만의 5h·주간 한도를 따로 보여주는데, 이 섹션은 플랜 한도가 아니라 파싱하지 않는다. 플랜에 5시간 창이 없으면 대표 사용량은 주간으로 표시된다
- 현재 사용량 신뢰도 계약은 반복 횟수로 낮은 값을 강제 채택하지 않는다. 감소 fresh가 5회 이어지면 조회 PTY를 재생성해 독립적으로 다시 확인하고, reset 시각이 전진한 새 창만 채택한다. Claude 2.1.251의 `seeded headers`·`seeded persisted`·갱신 실패·로딩·상세 endpoint 제한 문구는 숫자가 보여도 모두 비권위 관측으로 분류해 마지막 직접 확인값을 보존한다. 이 제한은 사용자 5시간/주간 quota 초과가 아니며 대시보드에는 `캐시`와 fallback 출처를 표시한다. 반복 fallback은 자동 조회를 2·4·8·최대 15분으로 백오프하고, fallback·로딩에서는 초기화 알림·keepalive를 실행하지 않는다. `Current session`이 0%인데 Resets가 없으면 숫자는 채택하지 않고 초기화 고정 턴은 보내며, 저장된 세션 리셋이 이미 지났으면 대시보드를 0%·5시간 뒤로 보정한다. 같은 리셋 날짜에서 0%로 떨어지면 2회 연속 관측 뒤 조회 터미널을 재시작하고, 재시작 후 조회도 0%면 확정한다. 리셋 시각이 바뀐 5시간 창은 바로 반영한다.
...
- 장기 세션의 수십 MB JSONL은 최초 한 번만 전체 파싱하고 이후 추가된 레코드만 증분 반영한다. 이미 연결된 활성 채팅의 주 기록과 Grok 턴·토큰 사이드카는 2초마다 빠르게 확인하되, 새 외부 세션을 찾는 전 계정 기록 저장소 재귀 탐색과 변경 없는 busy 고착 복구용 강제 재파싱은 30초마다 분리 실행한다. 실제 질문을 받은 미귀속 채팅은 기록 파일이 붙을 때까지 빠른 전체 탐색으로 자동 승격해 첫 응답 반영을 늦추지 않는다. 터미널 상태는 headless 화면에서 공유 판정해 반복 `tmux capture-pane` 프로세스와 일시 메모리 사용을 제한
- 별도 `설정` 탭에서 브라우저 알림 권한, 로그인 세션·비밀번호, CLI 계정·인증, Slack bot token·channel id, ntfy topic·서버 URL, signed outbound webhook, 유휴 채팅 종료 정책과 일회용 로그인을 관리한다. 다른 로그인 세션을 한 번에 폐기하거나 비밀번호 변경과 함께 전체 세션을 폐기할 수 있다. Slack·ntfy·webhook 설정을 저장하고 테스트 전송할 수 있으며, 작업 완료·권한 요청·사용량 한도 도달/초기화·터미널 종료 알림은 활성 채널 모두에 전송된다. Webhook endpoint 전체와 HMAC secret은 vault에만 암호화하고 host만 다시 표시한다
- 관리자는 설정 화면에서 숫자 8자리 일회용 로그인 코드를 발급할 수 있다. 코드는 원문 대신 해시로 저장되고 10분 안에 한 번만 사용할 수 있으며, 로그인 후 1시간 동안 일반 사용자 권한만 제공한다. 임시 세션은 실제 내부망에서 접속해도 숨김 파일·되돌릴 수 없는 작업용 내부망 capability를 얻지 못한다
- 정식 계정은 설정에서 표준 인증 앱용 TOTP MFA를 켤 수 있다. seed는 dataDir의 소유자 전용 key로 AES-256-GCM 암호화하고, 일회용 복구 코드 10개는 해시로만 저장한다. MFA 계정은 비밀번호 확인 뒤에도 짧은 pending challenge만 받고 TOTP 또는 복구 코드가 확인돼야 세션 cookie가 생긴다
- 세션은 절대 TTL과 별도로 기본 12시간 동안 HTTP/WebSocket 활동이 없으면 만료된다. 삭제·CLI 업데이트/롤백·프로세스 종료·계정/MCP 변경·Git push/PR merge 같은 중요 작업은 최근 본인 확인 창이 지나면 mutation 전에 거부되며, 설정에서 비밀번호와 선택적 MFA 코드로 다시 확인할 수 있다
...
- 서버 로그는 기본 3일치만 보존하고 6시간마다 오래된 파일을 정리(`WEB_AGENT_MANAGER_LOG_KEEP_DAYS`로 조절). 파싱 화면까지 남기는 디버그 레벨(`WEB_AGENT_MANAGER_LOG_LEVEL=debug`)로 오래 운용해도 디스크가 차지 않게 함 — 예전에는 서버 시작 때 한 번만 정리해 오래 켜둔 서버에서 계속 쌓였다
- 웹소켓 하트비트로 조용히 죽은 연결을 양쪽에서 감지 — 서버는 25초마다 ping을 보내 응답 없는 연결을 정리하고, 클라이언트는 하트비트가 70초 넘게 끊기면 소켓을 새로 연결. 모바일에서 NAT 유휴 타임아웃이나 셀룰러↔WiFi 전환으로 연결이 close 없이 끊겨도(앱을 켜둔 채 대기하는 동안 포함) 스스로 복구된다
...
- Agent Lab은 하나의 명령을 모델·스킬·권한·하네스·훅·예산 조건별 Variant로 저장하고 품질·성공률·벽시계 시간·토큰·비용·종료 상태를 비교한다. 실험은 산출물 계약(`code_change` 또는 `finding_report`)을 가지며, fixture는 리뷰 대상 커밋과 정답 결함 목록을 저장할 수 있다. `finding_report` 탐지 채점기는 정답을 사전식 순서(mustFind 재현율 → 오탐 → 위치·선택 항목 → 비용)로 비교하고, 사람 재심 판정을 다시 넣어 점수를 재계산할 수 있다. 리뷰 실험은 구조화 스키마를 Runtime에 넘기며 형식 실패는 run을 끝내지 않고 산출물 상태로 남긴다. `finding_report`는 세 공급자에 같은 계약 문구를 과제 뒤에 붙이고(파일을 읽은 뒤에만 보고, 작업 계획·진행 안내는 findings에 넣지 않음, 결함 없으면 빈 배열) run 스냅샷에 남기며, arm별 `promptPrefix`와는 섞지 않는다. 파일을 안 읽고 빈 배열을 낸 실행은 `toolCallCount`(tool_started만, 비교는 0 vs >0)로 관찰하고 승자 규칙에는 넣지 않는다. `Single`은 현재 설치 custom skills/공급자 built-in 기준선 각각에 선택 스킬을 더한 네 조건을 실행별 read-only native bundle로 만들고, 같은 비교 그룹의 모델·reasoning·권한·하네스·훅·예산·스킬 활성화 방식·Git HEAD를 고정한다. Claude는 자동 발견 외에 두 arm이 동일한 무상태 `SessionStart` 훅을 거치고 처리군에만 추가 스킬 본문을 넣는 강제 활성화를 선택할 수 있어, 훅 기반 스킬을 전역 설정 쓰기 없이 비교한다. `Orchestrator → Workers`, `Evaluator → Optimizer`는 primary/secondary Codex·Claude 모델, worker·반복·점수·무개선 한도와 내장 hook을 detached worktree에서 실행한다. 완료 run은 공급자별 CLI capability를 지키는 복수 evaluator의 블라인드 judgment와 `partial`을 보존하며, 우승 run은 accepted 사람 판정과 지표·경고를 포함한 불변 Agent preset 새 버전으로 승격한다. graph 공급자별 스킬 overlay, pairwise·통계 자동 반복과 일반 채팅 preset 실행은 후속 범위다. 전체 구조와 한계는 [에이전트 실험실 설계 문서](docs/agent-lab.md)에 정리한다
...
- 도구 탭에서 Claude/Codex별 commands·skills·marketplace·MCP 카탈로그 확인, Claude 프로젝트 `.mcp.json`과 Codex 사용자/프로젝트 `config.toml` 기반 MCP 추가·수정·삭제·Codex 활성화 토글 관리, provider CLI가 보고하는 연결 상태 전용 MCP도 read-only로 표시. 새 MCP env/header와 Slack bot token은 별도 AES-GCM vault에 암호화하고 설정 파일에는 공급자 공식 환경변수 참조만 남긴다([운영 절차](docs/operations/credential-vault.md)). 최초 카탈로그 응답 전에는 로딩 스피너를 표시하고 성공 응답 후에만 항목 없음 안내를 표시
- 설정에서 SQLite·프로필·일정·채팅 session mapping과 WAM 관리 key를 요청별 passphrase로 AES-256-GCM 암호화한 전체 백업을 생성·다운로드한다. 복구는 기존 설치를 덮어쓰지 않는 offline CLI만 제공하고 실제 임시 빈 환경 복구 훈련 명령으로 package와 key를 반복 검증한다([백업·복구 절차](docs/operations/full-backup-restore.md))
- 프로젝트 profile은 현재 8개 등록 프로젝트의 실제 wrapper/package script에 맞춘 검증 recipe·보호 작업을 미저장 초안으로 제안한다. owner가 불변 version을 활성화한 뒤에만 새 채팅에 pin하며 sandbox·승인·model·reasoning·추가 경로·tool 정책을 공급자 실행 경계에서 재검증한다([프로젝트 profile 운영 계약](docs/operations/project-profiles.md))
- 작업 탭의 카드 하나는 채팅 전체가 아니라 웹에서 보낸 입력 1회를 뜻한다. 같은 채팅에 여러 번 입력하면 카드도 여러 개 생기며, 새 입력이 시작되면 앞선 입력은 Completed로 이동하고 현재 응답만 Working에 남는다. task 원장은 Working/Needs input/Verifying/Failed/Completed/Scheduled로 복원하고 실행·idle·rate-limit·queue를 구분한다. 목표·완료 조건·체크포인트·다음 행동·예산을 보존하며, 계정 사용량/reset·provider capability·동시 실행 상한 기반 추천은 관리자가 별도 승인하기 전에는 공급자나 계정을 바꾸지 않는다([작업 보드·라우팅](docs/operations/task-board-routing.md))
- task 카드의 `웹 화면 확인`을 열면 프로젝트/worktree에서 실행 중인 로컬 HTTP 서버를 먼저 찾아 선택지로 보여준다. 찾지 못했을 때만 개발 서버 실행 후 `http://127.0.0.1:포트/` 입력을 요청한다. `증거 캡처`를 눌렀을 때만 headless Chrome이 viewport screenshot·정제된 console/network·접근성 증거를 task event에 연결한다. 외부 browser 요청, URL query, header/body/cookie/storage와 credential 원문은 수집하지 않는다([Live preview 절차](docs/operations/live-preview.md))
- 로컬 Git의 stage되지 않은 text diff를 hunk별 승인(stage)·거부(역적용)하고, 줄 의견을 동일 멱등 키의 기존 prompt 원장으로 현재 채팅에 재전송한다. 파일이 바뀐 stale snapshot과 binary/rename은 보수적으로 거부한다([Hunk 검토 절차](docs/operations/hunk-review.md))
- task screenshot을 명시적 기준선으로 고정하고 동일 viewport의 새 실제 Chrome 화면을 pixel diff PNG·변경 비율로 비교하며 axe 접근성 위반을 DOM/value 원문 없이 함께 남긴다([시각 회귀·접근성 절차](docs/operations/visual-regression-accessibility.md))
- 설정에서 pinned host key와 vault 보관 전용 private key, workspace root로 원격 worker를 등록하고 forwarding/password/TOFU 없이 고정 `capabilities --json`을 probe한다. 프로젝트 remote path는 root 아래로만 mapping하고 작업 카드의 별도 확인 뒤 선언된 `build|test|verify|preview`만 멱등 접수·상태 조회한다. 포함된 worker CLI는 owner-only 설정의 absolute executable·고정 argv recipe만 shell 없이 실행하며 임의 command/prompt/env API와 응답 불명 시 자동 재전송은 없다. 실제 host 설치와 프로젝트별 recipe 승인은 별도 운영 단계다([원격 worker 절차](docs/operations/remote-workers.md))
- 선택한 GitHub PR의 head/check/review/conflict/auto-merge 상태만 15초 간격으로 감시하고, 관리자 확인 뒤 동일 head·check 안전 조건을 다시 검증해 GitHub native `--auto --match-head-commit`을 예약·취소한다. branch protection과 merge queue를 우회하는 `--admin`은 사용하지 않는다([PR check·조건부 merge 절차](docs/operations/pr-check-auto-merge.md))
...
- Slack·ntfy 승인 요청·작업 완료·비정상 종료·사용량 한도 초기화 알림. Claude 5시간·전체 모델 주간, Codex 5시간·주간, Grok 주간 창을 계정별로 독립 추적하고, 정상 사용량 조회에서 실제 새 창 전환이 확인될 때 `5시간`·`주간` 종류를 명시해 각각 알림. 같은 창 종류 이름을 웹 대시보드·채팅 상태바·Android 위젯에도 사용한다. ntfy·FCM·Slack·브라우저가 서버의 동일한 제목·본문을 사용하며 작업 완료·한도 초기화 시 브라우저 알림 제공(설정 탭에서 권한 요청)
...
- 설정한 시간(기본 24시간) 동안 아무 활동이 없는 채팅 터미널을 10분 주기로 자동 종료. 관리자가 설정 탭에서 켜고 끄거나 기준 시간(1~720시간)을 조정하며, 작업 중이거나 리밋 재개를 기다리거나 승인 응답을 기다리는 채팅은 유휴 시간과 무관하게 종료하지 않음. 종료 사실은 알림 없이 감사 로그(`chat.idle_auto_stop`)에 시스템 주체로 기록
...
canary·일반 verification 실행과 재검증만 맡길 QA 계정은 관리자 계정을 공유하지 않고 테스트 전용 scope로 만든다. 이 계정의 모든 운영 변경 API는 기본 거부되며 실제 CLI 업데이트, 채팅 입력, live 승인, 파일·Git·설정 변경은 할 수 없다. 자세한 권한표는 `docs/operations/test-only-credentials.md`에 있다.
...
실제 CLI 후보 canary와 신규 채팅 단계 rollout은 기본 비활성이다. 공급자별 고정 harness, 전용 테스트 credential seed, 같은 후보 CLI binary를 소유자 전용 디렉터리에 준비한 뒤 `WEB_AGENT_MANAGER_PROVIDER_CANARY_HARNESS_DIR`, `WEB_AGENT_MANAGER_PROVIDER_CANARY_CREDENTIALS_DIR`, `WEB_AGENT_MANAGER_PROVIDER_CANDIDATE_CLI_DIR`을 설정한다. harness가 없으면 canary는 안전하게 blocked되고 candidate CLI 디렉터리가 없으면 단계 rollout 및 전체 승격 gate를 사용하지 않는다. credential seed는 운영 HOME이나 일반 계정과 공유하지 않는다. 상세 실행 계약과 JSON 형식은 `docs/operations/provider-cli-canary.md`를 참고한다.

Codex에는 production build가 만드는 `dist/canary/codex-app-server-canary.js`를 owner-only harness root의 `codex`로 복사해 쓸 수 있다. 전용 seed가 없으면 시작 전 blocked되며, 실제 실행은 app-server 로그인/thread/resume/고정 응답/approval decline/interrupt/rate-limit의 10단계를 검사하고 단계별 지연만 원장과 대시보드에 남긴다. 고정 모델 turn 최대 5회를 사용하므로 운영 계정이 아닌 사용량 제한된 테스트 계정으로만 실행한다.

Codex app-server 대화형 경로도 기본 비활성이다. read-only shadow의 7일 thread/turn gate가 통과한 환경에서만 `WEB_AGENT_MANAGER_CODEX_INTERACTIVE_TRANSPORT_CANDIDATE=1`, 비어 있지 않은 `WEB_AGENT_MANAGER_CODEX_INTERACTIVE_TRANSPORT_COHORT`, `WEB_AGENT_MANAGER_CODEX_INTERACTIVE_TRANSPORT_MAX_NEW_CHATS=1`처럼 세 값을 함께 설정한다. 그러면 설정 이후의 무기록 신규 Codex 채팅만 quota까지 구조화 경로를 사용하고 기존 채팅은 TUI를 유지한다. test_only 계정은 이 설정이나 채팅 입력을 바꿀 수 없다. ACK 유실·재시작에서는 자동 재전송하지 않는다. 운영 절차와 폴백 경계는 `docs/operations/codex-app-server-shadow.md`에 있다.
...
새 production build를 재시작하기 전 `npm run deploy:check`와 `npm run deploy:drill`을 실행한다. check는 운영 SQLite를 read-only로 열어 무결성, 적용 예정 table/column, busy·전이 중 채팅과 검증 중 task·ACK 전 command/run, build와 supervisor 세대를 JSON으로 확인한다. drill은 read-only online snapshot을 OS 임시 0700 dataDir에 만들고 current migration을 복제본에만 적용해 integrity, foreign key, durable table 건수, 전체 schema와 임시 test_only hash 검증을 수행한 뒤 삭제한다. `running` task와 `started` command는 tmux/원장으로 재시작 뒤 유지되므로 그 자체만으로 막지 않는다. blocker가 있으면 check가 종료 코드 2로 끝나며 두 명령 모두 운영 DB·상태·process를 변경하지 않는다. `migrationRequired=true`이면 기존 서버가 새 백업 API를 아직 제공하지 않는 경우에도 workspace 밖 owner-only 0600 파일의 absolute path를 `WEB_AGENT_MANAGER_BACKUP_PASSPHRASE_FILE`에 설정하고 `npm run backup:create`를 실행한다. 이 명령은 운영 DB를 read-only로 snapshot하고 암호화 package를 만든 뒤 임시 빈 환경의 실제 복구 검증까지 통과한 package 경로만 출력한다. passphrase를 별도 보관한 뒤 blocker가 0일 때만 아래 재시작 절차를 진행한다.
...
`test:resources`는 동일 브라우저 문서에서 주요 탭을 반복 전환하고 Chromium GC 전후의 JS heap·DOM·이벤트 리스너 증가를 검사한다. `test:tui`는 실제 Codex·Claude·Grok TUI를 띄워 사용량 명령을 직접 입력하고 화면 파싱을 검증한다. Claude가 fallback을 반환하면 실패로 오인하지 않고 해당 숫자가 신뢰값으로 노출되지 않는 계약을 반복 확인한다. `test:lifecycle`은 실제 PTY에 최소 대화를 입력해 새 세션을 저장하고 종료한 뒤 동일 ID resume를 검증하므로 CLI 사용량이 발생할 수 있다.
...
- 로그인은 IP 전체와 계정 전체 기준으로 연속 실패를 제한하고 실패·제한 이벤트를 비밀번호 없이 감사 로그에 남긴다. 제한 bucket은 원문 IP·아이디 대신 SHA-256으로 SQLite에 저장되어 서버 재시작 뒤에도 유지된다.
- 로그인 제한 상태와 인증 실패 감사 로그는 행 수·보존 기간 상한을 적용하며, 존재하지 않는 계정도 실제 계정과 같은 비밀번호 검증 비용을 사용한다. 세션·role·scope·연결 기기 권한이 바뀌면 열린 WebSocket도 메시지 처리 전 또는 1초 검사 주기 안에 종료된다. 운영 절차는 [로그인·세션 보안](docs/operations/session-security.md)을 따른다.
...
- WAM이 직접 받은 Slack·MCP·webhook 자격증명은 별도 master key의 AES-256-GCM vault에만 보관하고, 프로젝트·공급자 또는 전송별 짧은 내부 lease 동안만 전달한다. 원문 반환 API는 없으며 기존 공급자 로그인 저장소는 vault로 가져오지 않는다. Webhook은 공개 DNS HTTPS만 허용하고 검사한 IP를 실제 TLS 연결에 고정하며 redirect를 따르지 않는다.
- 전체 백업은 요청 passphrase를 저장하지 않는 인증 암호문이며 관리자·최근 재인증·신뢰 네트워크로 제한한다. 복구는 존재하지 않거나 빈 dataDir에만 원자 반영하고 이전 web session과 실행 상태를 부활시키지 않는다. 공급자 로그인 디렉터리와 프로젝트 저장소는 별도 백업 대상이다.
