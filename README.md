...
- 대시보드 런타임 카드에서 관리자가 Codex·Claude·Grok CLI를 공식 업데이트 명령으로 갱신할 수 있다. 성공 후 해당 공급자의 실행 중 채팅을 새 바이너리로 재시작하고 모델 카탈로그 캐시와 사용량 폴백 PTY를 정리해 direct 수집을 다시 시작한다. 진행 중인 응답이 중단될 수 있음을 확인창에 명시한다.
...
- Claude·Codex·Grok 사용량은 계정별 direct API/RPC를 60초마다 조회하고, 실패한 주기에만 임시 PTY로 기존 파서를 실행한 뒤 닫는다. 모델 목록도 공급자별 direct 경로를 우선하고 실패 시 TUI 메뉴로 폴백한다. 대시보드의 `수집기 재시작`은 모델 캐시·폴백 PTY를 정리해 다시 조회하며 실행 중인 채팅에는 영향을 주지 않는다.
- 새 사용량 창의 초기화 시각은 필요할 때 별도 임시 세션의 최소 턴으로 고정한다. Codex keepalive는 격리 `codex exec --json`으로 보내고 완결된 답과 `turn.completed`를 확인한다. 첫 요구는 1500자이며 최대 10회에 걸쳐 10000자까지 늘리고, 소진되면 알린다. 이미 1% 이상인 5시간 창에는 중복 전송하지 않는다.
- Codex Full reset 초기화권은 app-server에서 매분 개수·가장 이른 기한을 확인하고 변화 시 알린다. direct 조회 실패 시 상세 PTY 폴백은 최대 1시간에 한 번만 연다. 관리자는 사용 확인창을 거쳐 공식 app-server 경로로 초기화권을 사용할 수 있다. 사용량·기한은 한국 시간 기준 월·일·시·분으로 표시하고 `조회 원본`에서 direct 또는 폴백 결과를 확인할 수 있다.
...
`npm run release:package`는 production 빌드와 CycloneDX SBOM, SHA-256 체크섬을 포함한 다음 배포 파일을 `release/archives/`에 만든다(`<버전>`은 `package.json`의 현재 버전, 예: `0.7.0`).
