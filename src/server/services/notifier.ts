...
export interface Notifier {
  notify(eventId: string, eventType: string, text: string, presentation?: NotificationPresentation): Promise<void>;
}
...
export class NotificationHub implements Notifier {
  ...
  async notify(eventId: string, eventType: string, text: string, presentation?: NotificationPresentation): Promise<void> {
    await Promise.all(this.channels.map((channel) => channel.notify(eventId, eventType, text, presentation).catch(() => undefined)));
  }
}
